 ...
 * `#/personas`, `#/maintenance`. The hash is never sent to the server, so
 * the app works from any static host without SPA fallback config; the same
 * reason llama.cpp's webui uses `router: hash`.
 */
export type Route =
  { name: 'chat'; chatId: string | null } | { name: 'eltm' } | { name: 'personas' } | { name: 'maintenance' }
...
export function parseHash(hash: string): Route {
  ...
  if (path === '/maintenance' || path === '/maintenance/') return { name: 'maintenance' }
