...
class ReasoningRewriteHttpClient(private val delegate: HttpClient) : HttpClient {
    ...
    override fun execute(request: HttpRequest, parser: ServerSentEventParser, listener: ServerSentEventListener) {
        delegate.execute(request, parser, listener.rewritingEvents())
    }

    override fun execute(request: HttpRequest, listener: ServerSentEventListener) {
        delegate.execute(request, listener.rewritingEvents())
    }

    private fun ServerSentEventListener.rewritingEvents() = object : ServerSentEventListener {
        override fun onOpen(response: SuccessfulHttpResponse) {
            this@rewritingEvents.onOpen(response)
        }

        override fun onEvent(event: ServerSentEvent, context: ServerSentEventContext) {
            this@rewritingEvents.onEvent(event.rewriteReasoningDialect(), context)
        }

        override fun onEvent(event: ServerSentEvent) {
            this@rewritingEvents.onEvent(event.rewriteReasoningDialect())
        }

        override fun onError(error: Throwable) {
            this@rewritingEvents.onError(error)
        }

        override fun onClose() {
            this@rewritingEvents.onClose()
        }
    }
 ...
 */
class ReasoningRewriteHttpClientBuilder(private val delegate: HttpClientBuilder) : HttpClientBuilder {
