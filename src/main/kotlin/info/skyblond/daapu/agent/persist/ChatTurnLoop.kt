...
import info.skyblond.daapu.agent.chat.ChatStoreEntry
...
suspend fun runChatTurn(
     ...
     * proactive path (the reactive `context_exhausted` path still
     * compacts).
     */
    ...
    if (compactionTriggerFraction > 0 &&
        currentPromptTokens(chat) > model.contextLength * compactionTriggerFraction
    ) {
...
private suspend fun runHandRun(
    ...
    val request = HandRunRequest(
        ...
        tools = specs.takeIf { it.isNotEmpty() }?.map { HandToolSpec(it.name, it.description, it.schema) },
    ...
    return newChat to (terminal ?: throw HandUpstreamException("hand run ended without a terminal event"))
}
