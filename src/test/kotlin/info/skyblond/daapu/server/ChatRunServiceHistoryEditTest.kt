 ...
 * and everything after, WITHOUT SSTM extraction; fork: copy the prefix up to
 * a naturally finished assistant message into a new chat). Neither calls the
 * LLM, and truncate runs under the per-chat lock like a delete.
 */
class ChatRunServiceHistoryEditTest {
    ...
    fun `truncate rejects an out of bounds index`() = runBlocking {
        ...
        assertFailsWith<BadRequestException> { srv.truncateChat("chat-1", -1) }
        assertFailsWith<BadRequestException> { srv.truncateChat("chat-1", 6) }
    ...
    fun `truncate on an empty chat rejects any index`() = runBlocking {
        ...
        assertFailsWith<BadRequestException> { srv.truncateChat("chat-1", 0) }
    ...
    fun `fork on an empty chat rejects any index`() = runBlocking {
        ...
        assertFailsWith<BadRequestException> { srv.forkChat("chat-1", 0) }
    ...
    fun `fork rejects an out of bounds index`() = runBlocking {
        ...
        assertFailsWith<BadRequestException> { srv.forkChat("chat-1", -1) }
        assertFailsWith<BadRequestException> { srv.forkChat("chat-1", 6) }
        assertTrue(store.listChats().size == 1, "a rejected fork must not create a chat")
    }
    ...
    fun `fork rejects an index that is not a naturally finished assistant message`() = runBlocking {
        ...
        assertFailsWith<BadRequestException> { service(store2).forkChat("chat-1", 1) }
