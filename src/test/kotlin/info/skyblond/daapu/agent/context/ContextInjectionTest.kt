...
class ContextInjectionTest {
    ...
    fun `test localtime format`() {
        ...
        val injectionPart = contextInjection.generateInjection(
            InjectionSpec(
                time = ZonedDateTime.of(2026, 8, 5, 12, 34, 56, 789_000_000, java.time.ZoneOffset.ofHours(2)),
                eltmUpdated = false, relatedEntities = emptyList(), relatedNotes = emptyList()
            )
        )
        assertTrue {
    ...
    fun `test related entities and notes render under memories`() {
        ...
        val injectionPart = contextInjection.generateInjection(
            InjectionSpec(time = ZonedDateTime.now(), eltmUpdated = false, relatedEntities = hits, relatedNotes = notes)
        )
    ...
    fun `test the simple injection is rejected with mixed spec fields`() {
        ...
        // the mixed shape is unrepresentable now: generateInjection takes the
        // spec, so the all-or-nothing rule is enforced at its one entry point
    }
